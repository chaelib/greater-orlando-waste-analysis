...
-- in Greater Orlando?
--
-- Aggregates county-level data into one regional row per year.
...
-- ============================================================
-- Creates one regional row per year and calculates:
-- - Population
-- - MSW collected
-- - MSW landfilled
-- - MSW recycled
...
SELECT
    ...
    ROUND(
        SUM(MSW_Recycled_Tons)
        / SUM(MSW_Collected_Tons) * 100,
        2
    ) AS Regional_Recycling_Rate,

    ROUND(
        SUM(MSW_Collected_Tons)
        / SUM(Population),
        3
    ) AS Regional_MSW_Per_Capita
...
-- regional population?
--
-- Compares annual percentage changes in population and MSW.
...
--
-- Conclusion:
-- Waste generation does not consistently move with population growth.
-- There is substantial year-to-year variability.
...
--
-- MSW per capita is calculated as total regional MSW
...
-- Key observations:
-- Regional MSW per capita peaked at approximately
-- 2.069 tons/person in 2019.
--
-- It fell sharply in 2020, reached approximately
-- 2.062 tons/person in 2023, and declined to
-- approximately 1.941 tons/person in 2025.
...
-- ============================================================
-- Business Question:
-- Which counties generate the most waste relative to their
-- population, and which have the strongest recycling performance?
--
-- Calculates:
-- - Average MSW per capita
-- - Average recycled tons per capita
...
SELECT
    ...
    ROUND(
        AVG(MSW_Collected_Tons / Population),
        3
    ) AS Avg_MSW_Per_Capita,

    ROUND(
        AVG(MSW_Recycled_Tons / Population),
        3
    ) AS Avg_Recycled_Per_Capita,

    ROUND(
        SUM(MSW_Recycled_Tons)
        / SUM(MSW_Collected_Tons) * 100,
        2
    ) AS Overall_Recycling_Rate,

    ROUND(
        SUM(MSW_Landfilled_Tons)
        / SUM(MSW_Collected_Tons) * 100,
        2
    ) AS Overall_Landfill_Rate
...
-- Osceola County has the lowest MSW per capita but the
-- highest landfill rate.
...
--
-- Compares MSW collected and recycling rates at the
...
SELECT
    ...
    ROUND(
        (b.MSW_Collected_Tons - a.MSW_Collected_Tons)
        / a.MSW_Collected_Tons * 100,
        2
    ) AS MSW_Change_Percent,
    ...
    ROUND(
        b.Recycling_Rate - a.Recycling_Rate,
        2
    ) AS Recycling_Rate_Change_Points
...
--
-- This supports the Regional MSW Growth KPI.
...
SELECT
    ...
    ROUND(
        (b.Regional_MSW - a.Regional_MSW)
        / a.Regional_MSW * 100,
        2
    ) AS MSW_Change_Percent,

    ROUND(
        (b.Regional_Population - a.Regional_Population)
        / a.Regional_Population * 100,
        2
    ) AS Population_Change_Percent
...
--
-- Recycling-rate change is expressed in percentage points.
...
-- ============================================================
-- Business Question:
-- Which counties experienced waste growth that exceeded
-- or lagged behind population growth from 2017 to 2025?
--
-- The final metric measures the difference between MSW growth
-- and population growth in percentage points.
...
SELECT
    ...
    ROUND(
        (
            (b.MSW_Collected_Tons - a.MSW_Collected_Tons)
            / a.MSW_Collected_Tons
            -
            (b.Population - a.Population)
            / a.Population
        ) * 100,
        2
    ) AS MSW_Growth_Above_Population_Points
